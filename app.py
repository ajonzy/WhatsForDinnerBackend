...
class MealplanSchema(ma.Schema):
    ...
    rules = ma.Nested(multiple_rule_schema)
    shoppinglist = base_fields.Function(lambda fields: shoppinglist_schema.dump(filter(lambda shoppinglist: not fields.shoppinglists.is_sublist, fields.shoppinglists)[0] if len(fields.shoppinglists) > 0 else None))
    sub_shoppinglist = base_fields.Function(lambda fields: shoppinglist_schema.dump(filter(lambda shoppinglist: not fields.shoppinglists.is_sublist, fields.shoppinglists)[0] if len(fields.shoppinglists) > 1 else None))
